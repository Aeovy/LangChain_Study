...
class LLM_Model_async(LLM_Model):
    ...
    async def chat_async(self, qurey):
        if type(qurey)==str:
            message=[HumanMessage(qurey)]
        else:
            message=qurey
        ...
        async for chunk in self.LLM.astream(message):
            ...
            elif chunk.response_metadata["finish_reason"]=="function_call":
                tools_result=self.function_call(chunk)
                return self.chat_async(message)
            elif chunk.response_metadata["finish_reason"]!="":
                break
...
async def main():
    test_model=LLM_Model_async(model_type="openai",temperature=0.7)
    qurey="现在几点了？"
